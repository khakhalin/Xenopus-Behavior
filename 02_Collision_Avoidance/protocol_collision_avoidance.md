...
8. Quantify each trial as you go, as either a "success "(if the avoidance response was triggered) or a "failure", while also recording a video for further processing.

## Analysis

The simplest experimental design for collision avoidance involves comparing the rates at which avoidance responses are triggered in different treatment groups. This analysis requires minimal equipment, can be done by eye during the experiment, and later verified from the video (scored by another person, with only matching scores included in the final analysis). This approach is also appropriate for teaching labs. To further probe different aspects of brain development, sweep across several different values of circle speeds, sizes, or contrasts, and compare the curves of response probabilities between groups (Henriet 2017). 

A more sophisticated analysis requires tracking of tadpole position at every frame, which can be done manualy (ImageJ), or automatically,  using commercial (Noldus EthoVision; CleverSys AquaScan) or open source software, such as CeTrAn, C-trax and other solutions (Colomb 2012; Chao 2015). A perfect tracking of both the stimulus and the tadpole gives access to several more variables, including the peak swimming speed during an escape, an angle at which the tadpole turned during the maneuver, and the distance to the circle when the response is triggered (Khakhalin 2014). It also allows automated classification of responses into "successes" and "failures".

As not all software solutions can automatically track both the circle and the tadpole, another option is to add a diffused near-IR light source below the projection screen, and turn the camera to IR-only, by replacing a glass IR filter inside it with a stack of of thee colored (red, green, and blue) acrylic filters (Truszkowski 2017). In this configuration, the camera would only record the tadpole, but not the stimulus, which may provide enough information for some types of analysis.

The stimulation program presented here can also be used for multisensory experiments (Truszkowski 2017), as it can deliver a sound either when the circle hits the wall, or immediately before, or immediately after that. The description of this protocol however falls out of scope of this paper.
...
* Without a screen around the chamber, the tadpoles may be habituated to moving visual stimuli. Try to minimize movement.

# Discussion

A typical responsiveness for this protocol is 80% for healthy naive stage 49 tadpoles (Khakhalin 2014), down to 70% in dark-reared tadpoles (Ramirez-Vizcarrondo 2015), 50% in tadpoles adjusted to strong visual stimulation (Jang 2016); 40% in tadpoles raised in valproic acid (a psychoactive teratogenic drug; James 2015), and down to 20% in nutritionally restricted tadpoles (Khakhalin, unpublished). A power analysis shows that with 20 stimuli per tadpole (10 minutes of recording), base response probability of 70%, 20 animals in each group, and α = 0.05, one can detect a drop of responsiveness to 60% with power of β = 80%. A typical fast avoidance maneuver involves a sharp turn at ~90±40° performed at a distance of ~1 cm from the circle, followed by a brief acceleration to 12±4 cm/s. A slow "course correction" is produced in response to larger, slower circles, and involves a shallower turn by ~70±50° and to acceleration to ~7±4 cm/s (Khakhalin 2014). Together, this makes visual collision avoidance a powerful tool to detect and dissect abnormalities in the sensorimotor system of Xenopus tadpoles.
